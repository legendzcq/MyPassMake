...
@interface LGPhotoView () <UIScrollViewDelegate> {
    CGFloat _pageW;
}
...
@property (nonatomic, strong) NSArray<LGZoomingImageView *> *imageViews;
...
- (void)layoutImageViews {
    ...
    for (NSUInteger i = 0; i < self.imageViews.count; i++) {
        LGZoomingImageView *imageView = self.imageViews[i];
...
- (void)loadImageAtIndex:(NSInteger)index {
    ...
    for (NSInteger i = 0; i < count; i++) {
        LGPhotoModel *model = self.photoModels[i];
        LGZoomingImageView *imageView = self.imageViews[i];
        if (i == index) {
...
- (void)setCurrentImageView:(LGZoomingImageView *)currentImageView {
...
- (void)getImages {
    ...
    for (NSUInteger i = 0; i < count; i++) {
        LGZoomingImageView *imageView = [LGZoomingImageView new];
        LGPhotoModel *model = self.photoModels[i];;
