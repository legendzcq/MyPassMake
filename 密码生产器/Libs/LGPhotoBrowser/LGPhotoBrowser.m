...
#import "LGPhotoBrowser.h"
#import "LGPhotoCollectionView.h"
...
@interface LGPhotoBrowser () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout> {
    CGFloat _margin, _gutter;
}

@property (nonatomic, weak) LGPhotoCollectionView *collectionView;
@property (nonatomic, assign) CGSize photoSize;
@property (nonatomic, weak) LGBrowserToolBar *toolBar;
@property (nonatomic, strong) NSMutableArray *selectModels;

@end

@implementation LGPhotoBrowser

- (void)viewDidLoad {
    [super viewDidLoad];
    [self initParams];
    UICollectionViewFlowLayout *layout = [UICollectionViewFlowLayout new];
    LGPhotoCollectionView *collectionView = [[LGPhotoCollectionView alloc] initWithFrame:[self getCollectionViewFrame] collectionViewLayout:layout];
...
- (void)setupViews {
    LGBrowserToolBar *toolBar = [[LGBrowserToolBar alloc] initWithFrame:[self getToobarFrame]];
        ...
        switch (item.tag) {
            case SGBrowserToolButtonBack: {
                ...
                for (NSUInteger i = 0; i < weakSelf.selectModels.count; i++) {
                    LGPhotoModel *model = weakSelf.selectModels[i];
                    model.isSelected = NO;
                    [weakSelf reloadData];
                }
                break;
            ...
            case SGBrowserToolButtonAction: {
                [[[LGBlockActionSheet alloc] initWithTitle:@"Save To Where" callback:^(UIActionSheet *actionSheet, NSInteger buttonIndex) {
                    switch (buttonIndex) {
                        case 1:
            ...
            case SGBrowserToolButtonTrash: {
                [[[LGBlockActionSheet alloc] initWithTitle:@"是否删除此照片" callback:^(UIActionSheet *actionSheet, NSInteger buttonIndex) {
                    if (buttonIndex == 0) {
...
- (void)handleBatchSave {
    ...
    for (NSUInteger i = 0; i < count; i++) {
        LGPhotoModel *model = self.selectModels[i];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    LGPhotoModel *model = self.photoAtIndexHandler(indexPath.row);
    LGPhotoCell *cell = [LGPhotoCell cellWithCollectionView:collectionView forIndexPaht:indexPath];
    cell.model = model;
    return cell;
}
