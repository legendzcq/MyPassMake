...
@interface LGPhotoViewController ()

@property (nonatomic, assign) BOOL isBarHidden;
@property (nonatomic, weak) LGPhotoView *photoView;
@property (nonatomic, weak) LGPhotoToolBar *toolBar;
@property (nonatomic, strong)__block LGPhotoModel *currentPhoto;
@property (strong, nonatomic) AVPlayerViewController *playerVC;
@end

@implementation LGPhotoViewController
...
- (void)setupView {
    LGPhotoView *photoView = [[LGPhotoView alloc] initWithFrame:[self getPhotoViewFrame]];
    self.photoView = photoView;
    self.photoView.controller = self;
    self.photoView.index = self.index;

    [self.view addSubview:photoView];
    LGPhotoToolBar *tooBar = [[LGPhotoToolBar alloc] initWithFrame:[self getBarFrame]];
    self.toolBar = tooBar;
    [self.view addSubview:tooBar];
    WS();
    [self.toolBar setButtonActionHandlerBlock:^(UIBarButtonItem *sender) {
        switch (sender.tag) {
            case LGPhotoToolBarTrashTag:
                [weakSelf trashAction];
                break;
            case LGPhotoToolBarExportTag:
...
- (void)toggleBarState {
    ...
    LGPhotoModel * model =[self.photoView getcurrentPhoto];
...
- (void)trashAction {
    [[[LGBlockActionSheet alloc] initWithTitle:@"是否删除此照片" callback:^(UIActionSheet *actionSheet, NSInteger buttonIndex) {
        if (buttonIndex == 0) {
            ...
            LGPhotoModel * tempModel =[self.photoView getcurrentPhoto];
...
- (void)exportAction {
    [[[LGBlockActionSheet alloc] initWithTitle:@"保存到手机" callback:^(UIActionSheet *actionSheet, NSInteger buttonIndex) {
        if (buttonIndex == 1) {
